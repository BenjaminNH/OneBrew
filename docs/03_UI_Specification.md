...
*   **默认聚焦 (Default Focus)**: 默认展示用户当前勾选的参数清单，隐藏项由参数可见性控制，不再通过“展开更多参数”切换。
...
*   **全屏引导 (Android 优先)**: 首次启动进入全屏引导流程，允许跳过；跳过即使用默认模板与参数清单。
*   **两步流程**:
    1. **冲煮方式选择**：默认显示“手冲/意式”两项可勾选，至少保留 1 个。底部以灰色次要文字提示“使用其他冲煮方式？点此添加”。
    2. **参数清单**：按方式切换参数列表，系统预设参数在前，支持隐藏但不可删除；自定义参数入口在列表末尾，可新增/删除。仅支持 `数字/文本` 两类，单位由用户自填。
...
| 注水方式 | 文本 | 如分段/脉冲/连续 |
| 扰动方式 | 文本 | 如搅拌/旋转/不扰动 |
| 滤纸 | 文本 | 滤纸品牌/型号（如 Hario 无漂、Cafec Abaca） |
| 滤杯 | 文本 | 滤杯型号（如 V60、Kalita Wave、Origami） |
...
| 预浸时间 | 数字 | 若设备支持 |
| 研磨度 | 数字/文本 | 刻度值或描述 |
| 布粉 | 文本 | 记录布粉手法或工具（如 WDT、水平器） |
| 压粉 | 文本 | 记录压粉力度或工具（如 30lbs、平底压粉器） |
